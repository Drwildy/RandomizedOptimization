...
class GACustomNeuralNetwork(mlrose.NeuralNetwork):
...
def objective(trial):
    ...
    max_iters = trial.suggest_int('max_iters', 100, 300)
    mutation_prob = trial.suggest_categorical('mutation_prob', [0.1, 0.2, 0.3, 0.4])
    pop_size = trial.suggest_int('pop_size', 50, 500)
    

    model = GACustomNeuralNetwork(max_iters=max_iters, hidden_nodes=hidden_nodes, activation=activation, learning_rate=learning_rate_init, mutation_prob=mutation_prob, pop_size=pop_size, random_state=60)
...
model = GACustomNeuralNetwork(hidden_nodes=(10,), learning_rate=0.001, mutation_prob=0.1, pop_size=263, max_iters=271, max_attempts=10, random_state=60)
model.get_params()   
...
# fig2 = optuna.visualization.plot_slice(study, params=['hidden_nodes', 'learning_rate_init', 'mutation_prob', 'pop_size', 'max_iters'])
# fig2.show()
