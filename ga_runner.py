...
def knapsack():
    ...
    mutation_list = [0.1, 0.1, 0.2]
    popSize_list = [10, 70, 50]


    for prob, mutation, popSize in zip(problems, mutation_list, popSize_list):

        rhc = mlrose.GARunner(problem=prob,
        ...
        print(best_runs)
        best_mutation = best_runs[best_runs['Iteration'] == best_runs['Iteration'].min()].iloc[0]
        best_curve = df_run_curves[df_run_curves['Mutation Rate'] == best_mutation['Mutation Rate']]
...
def fourPeaks():
    ...
    # Corresponding number of restarts for each problem
    mutation_list = [0.3, 0.2, 0.2]
    popSize_list = [20, 40, 100]
