...
class RHCCustomNeuralNetwork(mlrose.NeuralNetwork):
...
def objective(trial):
    ...
    restarts = trial.suggest_int('restarts', 1, 50)

    model = RHCCustomNeuralNetwork(max_iters=max_iters, hidden_nodes=hidden_nodes, activation=activation, learning_rate=learning_rate_init, restarts=restarts,  random_state=60)
...
model = RHCCustomNeuralNetwork(activation='tanh', hidden_nodes=(10,5), learning_rate=.75, restarts=32, max_iters=358, max_attempts=10, random_state=60)
model.get_params()   
...
# fig2 = optuna.visualization.plot_slice(study, params=['hidden_nodes', 'learning_rate_init', 'restarts', 'max_iters'])
# fig2.show()
